...
|---|---|---|---|---|
| E1 | context_lightweight | Lightweight | `think=False` | Tier persona (company + product + role) · user message includes month name only |
...
**Conclusion:** The failure is not a prompting problem and not a model capability problem. Stateless agents without inventory state cannot self-correct accumulated drift. More parameters and richer context do not help.
