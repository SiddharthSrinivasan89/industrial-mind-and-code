...
-----
    ...
    Or create your own .env.local with the same keys. The committed sarvam env
    files document the V2a/V2d temperature and token settings.

Usage
-----
    # Local backend (llama-server / OpenAI-compatible endpoint)
    python run_experiment.py --experiments E1 E2 --conditions context --runs 10 --env .env.local
...
-----------------
    ...
    E1         — lightweight tier (use --conditions context for canonical V2a runs)
    E2         — reasoning tier (use --conditions context for canonical V2a runs)
    E4         — optional OSS reasoning extension hook (not part of canonical V2a)
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["blind", "context"],
        help="Filter to only these prompt conditions (default: run all). "
             "Baselines are always included regardless of this filter.",
    )
    ...
    for exp_name in args.experiments:
        ...
        specs = EXPERIMENTS[exp_name]
        if args.conditions:
            specs = [s for s in specs if s["policy"] != "llm" or s["condition"] in args.conditions]
