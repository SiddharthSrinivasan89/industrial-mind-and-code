...
| Factor | Levels |
|---|---|
| Context | Blind (numeric state only: demand, on-hand inventory, backlog, in-transit orders, lead time — no identity, product, or calendar) vs Context (same numeric state plus company, product, supply chain role, calendar month) |
...
- Demand series: 13 periods of real-pattern synthetic Vecta despatch targets (Dec 2024 – Dec 2025), with 12 active ordering periods and a period-13 close-out
...
Results are written to `results/raw/` (per-run JSON), `results/aggregated/` (per-config summary JSON), and `results/experiment.log`.
