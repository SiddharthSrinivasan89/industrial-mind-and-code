...
*Disclaimer: This is self-funded, independent research for academic purposes. The dataset is the Microsoft Azure Predictive Maintenance sample, obtained from Microsoft's `sqlworkshops` repository; this project relies on that repository's MIT license as covering the dataset (the retired original Azure AI Gallery terms could not be recovered).*

## TL;DR

- **The task:** Classify a known machine breakdown to one of four components using text-rendered summaries of pre-failure sensor and error logs.
...
- **Contamination probe (descriptive, n=51):** with disguised component names, the accuracy gaps were 0.000 for gpt-5.4 (90% CI [−0.063, 0.063]), +0.059 for gpt-oss:120b (CI [0.017, 0.120]), and −0.039 for nemotron-3-super (CI [−0.104, 0.021]); no pre-declared cap was triggered. The probe cannot establish invariance or absence of contamination at this size.
- **Conclusion:** On this specific, simulated dataset, a trained classical model outperformed every LLM configuration with non-overlapping confidence intervals, and a one-line rule had a higher point estimate than every LLM configuration (no formal paired superiority test compared the rule with the with-history LLMs).

## Evaluation design

I investigated whether large language models could accurately classify failed components from text-rendered summaries of pre-failure sensor data and error logs, and whether they would outperform standard deterministic baselines. 
...
I also hypothesized that the classical champion would outperform the LLMs even when they were provided with historical context, which the results confirmed. The logistic regression model achieved a near-perfect macro-F1 of 0.995. Even when the LLMs were supplied with training-period summary statistics—which narrows but does not close the information asymmetry, as the logistic regression model learns from labeled examples over the full joint distribution—their performance peaked between 0.855 and 0.908. The confidence intervals between the classical champion and the LLM models do not overlap.

Providing the LLMs with historical context yielded a substantial improvement, adding between 0.47 and 0.52 macro-F1 points across all tested models. However, even with this statistical context, no LLM configuration exceeded the 0.923 macro-F1 point estimate of the deterministic recent-error rule. I ran no formal paired superiority test between the rule and the with-history models, and their confidence intervals overlap; the precise observation is that the rule's point estimate was higher and the models did not exceed it.

The disguised-names probe is descriptive by design. The paired accuracy gaps were 0.000 for gpt-5.4 (90% CI [−0.063, 0.063]), +0.059 for gpt-oss:120b (CI [0.017, 0.120] — consistent with a minor reliance on the original labeling conventions), and −0.039 for nemotron-3-super (CI [−0.104, 0.021]); no pre-declared cap rule was triggered. At 51 events, and with a single label permutation, the probe cannot establish invariance or the absence of contamination; a design-time power simulation showed equivalence testing is infeasible at this size, and the prescribed observed-data minimum-detectable-effect calculation was not run (a disclosed protocol deviation).

## Conclusion

On this near-deterministic simulator data, the performance ordering by point estimate is clear. The frozen classical champion performed best (logistic regression, 0.995; it won the pre-specified tune-stage selection — the exploratory gradient-boosting alternative scored 1.000 on the same evaluation but was not the frozen champion), followed by the recent-error rule (0.923), the with-history LLMs (0.855–0.908), the oldest-component heuristic (0.467), the zero-shot LLMs (0.355–0.440), and the no-evidence majority guess (0.140).

The observation that these LLMs, reading text-rendered summaries of the pre-failure evidence, did not outperform a basic heuristic is a key finding. For the models, prompts, and evidence representation tested here, strong performance came with access to the training-period statistics rather than from the zero-shot condition; this describes these runs, not inherent model capability.
