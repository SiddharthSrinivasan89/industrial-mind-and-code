...
def _stream_call(client, model, messages, max_tokens, temperature, think=True, top_p=None):
    """
    ...
    think=False disables thinking mode for llama.cpp servers via extra_body.
    top_p=None means let the server default apply (llama.cpp default=0.95).
    Pass top_p=1.0 to match GGUF model card recommendation for reasoning tasks.
    """
    ...
    # Think flag varies by model architecture:
    #   sarvam-30b  (bailingmoe2): extra_body={"think": False} to suppress
    #   sarvam-105b (deepseek2):   extra_body={"enable_thinking": False} to suppress
    # THINK_FLAG_STYLE env var selects which mechanism to use. Default: "think" (30b).
    think_style = os.environ.get("THINK_FLAG_STYLE", "think")
    if think:
        extra = {}
    elif think_style == "enable_thinking":
        ...
        try:
            if attempt <= 2:
                raw, latency_ms, ttft_ms, prompt_tokens, completion_tokens = call_with_backoff(
                    _stream_call,
                    client, model, messages, max_tokens, temperature, think, top_p,
                    max_retries=10,
                    context=ctx,
                )
            else:
...
def get_ss_multiplier(
    ...
    """
    Hybrid interface: ask LLM for a safety stock multiplier (non-streaming).

    Uses non-streaming to avoid an Ollama quirk where reasoning models
    (including gpt-oss:120b) occasionally return all tokens in the
    delta.reasoning field with empty delta.content during streaming,
    yielding raw=''. Non-streaming reliably returns content for this model.

    Returns dict with keys matching get_order_decision() except:
      safety_stock_multiplier (float) instead of order_quantity (int)
      ttft_ms is always 0.0 (not measurable without streaming)
    """
    client = _build_client()
    model = _model_name(model_tier, model_name)
    max_tokens = _max_tokens(model_tier)
    ...
    think_style = os.environ.get("THINK_FLAG_STYLE", "think")
    extra: dict = {}
    if think_style == "enable_thinking":
        extra = {"enable_thinking": False}
    elif think_style != "none":
        extra = {"think": False}
    ...
    def _non_stream_call(client, model, messages, max_tokens, temperature, extra):
        t0 = time.perf_counter()
        ...
        return raw, lat, pt, ct

    for attempt in range(1, 4):
        try:
            if attempt <= 2:
                raw, latency_ms, prompt_tokens, completion_tokens = call_with_backoff(
                    _non_stream_call,
                    client, model, messages, max_tokens, temperature, extra,
                    max_retries=10,
                    context=ctx,
                )
            else:
            ...
            if not isinstance(parsed, dict):
                raise TypeError(f"Expected JSON object, got {type(parsed).__name__}")
            ss_mult = float(parsed["safety_stock_multiplier"])
            rationale = str(parsed.get("rationale", ""))
