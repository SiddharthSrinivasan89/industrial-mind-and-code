...
def _stream_call(client, model, messages, max_tokens, temperature, model_tier):
    ...
    # Use max_completion_tokens for all Azure models — reasoning models (o4-mini)
    # reject max_tokens with a 400. max_completion_tokens is supported across all
    # Azure OpenAI generations.
    # Temperature: o4-mini does not accept custom temperature and returns a 400 if
    # sent. Do not pass temperature for Azure to keep the call compatible across
    # all generations; stochasticity is preserved via model defaults.
...
def _non_stream_call(client, model, messages, max_tokens, model_tier):
    """
    Non-streaming Azure OpenAI call — used for get_ss_multiplier.

    Reasoning models (o4-mini) intermittently return empty delta.content
    when called with streaming after a 429/retry cycle.
    The non-streaming path waits for the full response and reliably
    captures content regardless of reasoning token usage.

    Returns the same 7-tuple as _stream_call with ttft_ms always 0.
    """
    kwargs = dict(
        model=model,
        messages=messages,
    ...
    return raw, latency_ms, 0.0, prompt_tokens, completion_tokens, reasoning_tokens, cached_tokens
...
def get_ss_multiplier(
    ...
    """
    Hybrid interface: ask Azure OpenAI for a safety stock multiplier.

    Same structure as get_order_decision() but parses safety_stock_multiplier
    (float) instead of order_quantity (int).
    """
    client = _build_client()
    model = _model_name(model_tier, model_name)
    max_tokens = _max_tokens(model_tier)
        ...
        try:
            if attempt <= 2:
                raw, latency_ms, ttft_ms, prompt_tokens, completion_tokens, \
                    reasoning_tokens, cached_tokens = call_with_backoff(
                        _non_stream_call,
                        client, model, messages, max_tokens, model_tier,
                        max_retries=10,
                        context=ctx,
                    )
            else:
