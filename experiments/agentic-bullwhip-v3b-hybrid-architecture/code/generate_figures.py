...
Generates 6 figures from the experiment results:
  ...
  fig1_ovar_stockouts.png    — OVAR vs stockout scatter (hybrid conditions vs baselines)
...
CONDITION_LABELS = {
    ...
    "exp_smoothing":        "Exp smoothing (benchmark)",
...
def fig1_ovar_stockouts(summaries: list[dict], out_dir: Path) -> None:
    """
    Joint OVAR vs stockout scatter. Key question: did hybrid beat exp_smoothing and hybrid_control?
    Success zone is determined by exp_smoothing results from this experiment.
    """
    fig, ax = plt.subplots(figsize=(10, 7))
    stats = _get_condition_stats(summaries)

    # Success zone: below exp_smoothing benchmark
    bench_ovar = stats.get("exp_smoothing", {}).get("chain_ovar", {}).get("mean", 1.0)
    ...
    markers = {
        ...
        "hybrid_blind_local":    ("o", C_LOCAL,     "H-Blind / local (nemotron-3-super:120b)"),
...
def fig3_run_variance(records_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    Includes success zone band (below exp_smoothing benchmark).
    """
...
def fig5_order_series(records_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    Three subplots, one per tier. Shows how hybrid order quantities compare to exp_smoothing.
    """
    from metrics import compute_ovar, compute_chain_ovar
