...
Each tier has a unique, named company identity with role-specific framing. The agent also receives the calendar month name — but not the year. This represents a realistic enterprise deployment where each agent knows who it is, what it makes, and what time of year it is operating.
...
### 4.3 Published Backend-Specific Run Bundles

The published March 2026 result set is built from four backend-specific two-condition bundles:

| Bundle | Conditions | Model | Backend |
|---|---|---|---|
| E1 Azure | `blind_lightweight`, `context_lightweight` | gpt-4.1-mini | Azure OpenAI |
| E1 Local | `blind_lightweight`, `context_lightweight` | phi4:14b | Local / Ollama |
| E2 Azure | `blind_reasoning`, `context_reasoning` | o4-mini | Azure OpenAI |
| E2 Local | `blind_reasoning`, `context_reasoning` | gpt-oss:120b | Local / Ollama |

This yields **8 backend-specific model-condition cells** and **11,520 total LLM calls** in the published comparison. The codebase also retains an optional `E4` extension hook for OSS reasoning comparisons, but the archived `results/` folder and the published report are based on the four bundles above.
...
## 7. Published Run Bundles

| # | Name | Primary question | LLM calls |
|---|---|---|---|
| 1 | E1 Azure | How does frontier lightweight perform in blind vs context conditions? | 2 × 20 × 24 × 3 = 2,880 |
| 2 | E1 Local | How does local lightweight perform in blind vs context conditions? | 2 × 20 × 24 × 3 = 2,880 |
| 3 | E2 Azure | How does frontier reasoning perform in blind vs context conditions? | 2 × 20 × 24 × 3 = 2,880 |
| 4 | E2 Local | How does local reasoning perform in blind vs context conditions? | 2 × 20 × 24 × 3 = 2,880 |
| | | **Published total** | **11,520 LLM calls** |

E3 remains a synthesis layer over these conditions rather than a separately executed run bundle. The optional `E4` code path is not part of the published March 2026 result set.
