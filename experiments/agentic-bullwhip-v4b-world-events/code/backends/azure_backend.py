"""
Azure OpenAI backend for V4 WorldEvents.

Public interface:
  get_intent_decision() — called by agent_interface.get_intent_class(); validates `intent` field
...
def get_intent_decision(
    ...
    """
    Call Azure and return intent classification + telemetry.

    The `intent` field must be one of the five valid class strings.
    If the parsed intent is not in _VALID_INTENTS, RuntimeError is raised so
    agent_interface.get_intent_class() can apply the NEUTRAL fallback.
    """
    ...
    try:
        ...
        raise RuntimeError(f"Missing or invalid intent field in response — {ctx}") from exc
