...
V2/V2a established that autonomous LLM agents produce 8× worse OVAR than
exponential smoothing. V2's Conclusion recommended hybrid architectures where
LLMs adjust the *parameters* of a deterministic policy rather than issuing orders
directly. This module implements and tests that recommendation.
...
def policy_exp_smoothing(
    ...
    This is the V2 benchmark baseline (OVAR 0.54, 5 stockouts). Used here for the
...
def policy_smoothed_out_with_ss(
    ...
    architectural baseline separate from V2's pure exp_smoothing (OVAR 0.54).
...
def run_simulation(
    ...
    policy            : "hybrid"        — LLM parameterises exp_smoothing safety stock
                        "llm"           — legacy: LLM decides order quantity directly (V2 behaviour)
                        "naive"         — passthrough heuristic
                        "order_up_to"   — forecast-based OUT heuristic
                        "exp_smoothing" — exponential smoothing heuristic (V2 benchmark)
    ...
    for _, row in active_periods.iterrows():
        ...
        for tier in TIERS:
            ...
            if policy == "hybrid":
                ...
                # Legacy autonomous LLM ordering (V2 behaviour — included for reference)
