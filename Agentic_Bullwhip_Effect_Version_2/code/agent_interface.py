...
def build_user_prompt(
    ...
    # Context condition only: tell the agent what month it is.
    # Blind condition deliberately omits this — that is the experimental treatment.
...
def get_order_decision(
    ...
    temperature : Explicit temperature override. Resolved by simulation.py based on
                  condition + model_tier (blind=0.0, context_lightweight=0.3,
                  reasoning=from env). Falls back to env var if not provided.
